...
root = osp.join(osp.dirname(osp.realpath(__file__)), '../..', 'data', 'dataset','ogbn-products')
...
model = GAT(dataset.num_features, 64, dataset.num_classes, num_layers=3,
            heads=4).to(device)
...
def train(epoch):
    ...
    # 每个epoch记录一次模型参数，最终只留下精度最高的模型参数
...
for run in range(1, 11):
    ...
    for epoch in range(1, 101):
        ...
        if acc > max_acc:
            ...
            model_save_path = osp.join(osp.dirname(osp.realpath(__file__)),f'ogbn_products_GAT_{train_loader.batch_size}.pth')
